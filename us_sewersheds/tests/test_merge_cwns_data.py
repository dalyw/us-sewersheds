...
def test_load_cwns_data(data_dir, expected_keys):
    """Test that load_cwns_data returns
    expected data structure with correct keys."""
...
def test_load_and_merge_cwns_data(data_dir, state, expected_columns):
    ...
    merged DataFrame with expected structure."""
    ...
    for col in expected_columns:
        assert col in result.columns, f"{col} should be present in merged data"

    if state:
        assert all(
            result["STATE_CODE"] == state
        ), f"All facilities should be in state {state}"

    # Check that there are no null values in key columns
    assert not result["CWNS_ID"].isnull().any(), "CWNS_ID has null values"
    assert (
        not result["FACILITY_NAME"].isnull().any()
    ), "FACILITY_NAME should not have null values"
...
def create_test_discharges_df(test_case):
    """Helper function to create test discharges
    DataFrame based on test case."""
...
@pytest.mark.parametrize(
    ...
    [
        ("basic_connections", [("1002", "1001"), ("1005", "1003")]),
        ("no_connections", []),
        ("single_connection", [("1002", "1001")]),
    ],
)
def test_build_sewershed_map(test_case, expected_connections):
    """Test that build_sewershed_map
    returns expected sewershed mapping structure."""
    ...
    for sewershed_id, sewershed_data in result.items():
        assert isinstance(
            sewershed_data, dict
        ), f"Sewershed {sewershed_id} should be a dictionary"
        assert (
            "nodes" in sewershed_data
        ), f"Sewershed {sewershed_id} should have 'nodes' key"
        assert (
            "connections" in sewershed_data
        ), f"Sewershed {sewershed_id} should have 'connections' key"
        assert isinstance(
            sewershed_data["nodes"], set
        ), f"Sewershed {sewershed_id} nodes should be a set"
        assert isinstance(
            sewershed_data["connections"], list
        ), f"Sewershed {sewershed_id} connections should be a list"
        ...
        for from_id, to_id in expected_connections:
            has_connection = any(
                any(
                    ...
                    for conn in sewershed["connections"]
                )
                for sewershed in result.values()
            )
            assert has_connection, f"Should have {from_id} to {to_id}"
    else:
        # For no_connections case, verify no connections exist
        total_connections = sum(
            len(sewershed["connections"]) for sewershed in result.values()
        )
        assert (
            total_connections == 0
        ), "Should have no connections for no_connections test case"
